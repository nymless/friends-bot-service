...
1. Handlers (Telegram commands) — `run_with_unit_of_work(callback)`:
   ...
   - passes uow into callback (use cases receive uow.users, uow.games, …);
   - on connection/SQLAlchemy errors logs and raises DatabaseUnavailableError;
   - the handler catches that and sends the user message from texts/system.py.
...
async def run_with_unit_of_work(
    ...
    """
    Opens one unit of work per call; raises DatabaseUnavailableError on DB failure.
    """
