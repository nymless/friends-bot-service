...
from friends_bot_service.usecases.stats.show_stats import (
    ShowStatsOutcome,
    ShowStatsResult,
)
...
async def test_show_statistics_reports_empty_leaderboard():
    ...
    with patch(
        "friends_bot_service.handlers.stats.run_with_unit_of_work",
        new=AsyncMock(side_effect=invoke_run_with_unit_of_work),
    ):
        with patch(
            "friends_bot_service.handlers.stats._show_stats.execute"
        ) as show_stats:
            show_stats.return_value = ShowStatsResult(
...
async def test_show_winner_statistics_returns_early_when_user_is_missing():
    ...
    with patch(
        ...
        with patch(
            ...
            show_stats.return_value = ShowStatsResult(
                outcome=ShowStatsOutcome.USER_MISSING
            )
...
async def test_stats_commands_delegate_to_show_statistics(handler, game_type):
    ...
    with patch(
        ...
        with patch(
            ...
            show_stats.return_value = ShowStatsResult(outcome=ShowStatsOutcome.EMPTY)
            await handler(message, bot, "upd-1")

    run_uow.assert_awaited_once()
    show_stats.assert_awaited_once()
    command = show_stats.await_args.args[0]
    assert command.bot_id == 30
    assert command.chat_id == 10
    assert command.game_type == game_type
